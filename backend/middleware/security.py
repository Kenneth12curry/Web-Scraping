...
class SecurityMiddleware:
    ...
    def _register_security_checks(self, app):
        ...
        def security_checks():
            ...
            if self._has_suspicious_headers(request.headers):
                ...
                return (
                    jsonify({"success": False, "message": "Requête suspecte détectée"}),
                    400,
                )
            ...
            if (
                ...
                logger.warning(
                    f"Contenu trop volumineux de {request.remote_addr}: {request.content_length} bytes"
                )
                return (
                    jsonify({"success": False, "message": "Contenu trop volumineux"}),
                    413,
                )
    ...
    def _is_suspicious_user_agent(self, user_agent):
        ...
        suspicious_patterns = [r"bot", r"crawler", r"spider", r"scanner", r"nmap"]
...
def require_https(f):
    ...
    def decorated_function(*args, **kwargs):
        if not request.is_secure and os.getenv("FLASK_ENV") == "production":
            return (
                jsonify({"success": False, "message": "HTTPS requis en production"}),
                403,
            )
