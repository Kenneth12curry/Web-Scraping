...
class Config:
    ...
    USER_AGENT = os.getenv(
        "USER_AGENT",
        "Mozilla/5.0 (compatible; FindataBot/1.0; +https://findata.local/bot)",
    )
    ...
    def validate_config(cls):
        ...
        if cls.ADMIN_PASSWORD == "admin123":
            warnings.append(
                "ATTENTION: Mot de passe admin par défaut détecté - Changez-le en production !"
            )
...
class ProductionConfig(Config):
    ...
    def validate_credentials(cls):
        if (
            not cls.ADMIN_USERNAME
            or not cls.ADMIN_PASSWORD
            or cls.ADMIN_PASSWORD == "admin123"
        ):
            raise ValueError(
                "Les identifiants admin doivent être configurés et sécurisés en production"
            )
        if not cls.SENTRY_DSN or cls.SENTRY_DSN == "your_sentry_dsn_here":
            raise ValueError("SENTRY_DSN doit être configuré en production")
        if not cls.GROQ_API_KEY:
            raise ValueError("GROQ_API_KEY doit être configuré en production")
        if not cls.SCRAPEDO_API_KEY or len(cls.SCRAPEDO_API_KEY) < 10:
            raise ValueError(
                "SCRAPEDO_API_KEY doit être configuré et valide en production"
            )
        if not cls.SMTP_USERNAME or not cls.SMTP_PASSWORD:
            raise ValueError(
                "Les identifiants SMTP doivent être configurés en production"
            )
        return True
        return True
