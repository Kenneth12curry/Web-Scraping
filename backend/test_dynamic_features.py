...
def test_weather_api():
    ...
    try:
        ...
        weather_response = requests.get(
            f"{BASE_URL}/dashboard/weather", headers=headers
        )
...
def test_notifications_api():
    ...
    try:
        ...
        notifications_response = requests.get(
            f"{BASE_URL}/dashboard/notifications", headers=headers
        )
        ...
        if notifications_response.status_code == 200:
            ...
            print(
                f"❌ Erreur HTTP {notifications_response.status_code} pour les notifications"
            )
            return False
...
def test_analytics_api():
    ...
    try:
        ...
        analytics_response = requests.get(
            f"{BASE_URL}/dashboard/analytics", headers=headers
        )
        ...
        if analytics_response.status_code == 200:
            ...
            if analytics_data["success"]:
                ...
                print(
                    f"   📊 Données graphiques: {len(analytics_data.get('chart_data', {}).get('labels', []))} jours"
                )
                print(
                    f"   🌐 Top domaines: {len(analytics_data.get('top_domains', []))}"
                )
                print(
                    f"   📝 Activité récente: {len(analytics_data.get('recent_activity', []))}"
                )
                return True
            else:
