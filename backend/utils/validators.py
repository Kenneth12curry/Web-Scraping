...
def validate_scraping_params(params: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    if method not in valid_methods:
        return (
            False,
            f"Méthode invalide. Méthodes autorisées: {', '.join(valid_methods)}",
        )
...
def sanitize_input(text: str, max_length: int = 1000) -> str:
    """
    Nettoyer un texte d'entrée en supprimant les balises HTML et les caractères de contrôle.
    Pour une désinfection XSS robuste pour le contenu rendu dans le frontend,
    utilisez une bibliothèque dédiée comme Bleach.
    """
    if not text:
        return ""
