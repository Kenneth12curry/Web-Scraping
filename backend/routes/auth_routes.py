...
from utils.helpers import get_user_by_email, create_password_reset_token, verify_password_reset_token, update_user_password, mark_token_as_used
...
def register():
    ...
    try:
        ...
        insert_query = """
            INSERT INTO users (username, email, password_hash, first_name, last_name, company, subscription_plan, monthly_requests_limit, monthly_requests_used, created_at)
            VALUES (%s, %s, %s, %s, %s, %s, 'free', 30, 0, NOW())
        """
        mysql_connector.execute_query(insert_query, (username, email, password_hash, first_name, last_name, company))
        ...
        try:
            ...
            if user_id:
                ...
                insert_query = """
                    ...
                    VALUES (%s, %s, %s, %s)
                """
                mysql_connector.execute_query(insert_query, (user_id, ip_address, user_agent, 'success'))
        except Exception as log_exc:
            logger.warning(f"Impossible d'enregistrer l'historique de connexion: {log_exc}")
    ...
    try:
        ...
        return jsonify({
            ...
            'profile': {
                ...
                'monthly_requests_limit': user.get('monthly_requests_limit', 30),
...
def update_profile():
    ...
    try:
        ...
        existing_user = mysql_connector.execute_query(check_query, (email, current_user))
    ...
    try:
        ...
        return jsonify({
            ...
            'config': {
                ...
                'scrapedo_token': user.get('scrapedo_api_key')[:10] + '...' if user.get('scrapedo_api_key') else None,
                'groq_token': user.get('groq_api_key')[:10] + '...' if user.get('groq_api_key') else None
            }
        }), 200
...
def update_api_config():
    ...
    try:
        ...
        update_query = """
            ...
            WHERE username = %s
        """
        mysql_connector.execute_query(update_query, (
...
def export_user_data():
    ...
    try:
        ...
        recent_requests_query = """
            ...
            LIMIT 50
        """
        recent_requests = mysql_connector.execute_query(recent_requests_query, (user_id,))
        ...
        scraping_history_query = """
            ...
            LIMIT 20
        """
        scraping_history = mysql_connector.execute_query(scraping_history_query, (user_id,))
        ...
        export_data = {
            ...
            'usage_stats': usage_stats[0] if usage_stats and len(usage_stats) > 0 else {
                'total_requests': 0,
...
def delete_account():
    ...
    try:
        ...
        if confirmation != 'DELETE':
            return jsonify({'success': False, 'message': 'Confirmation invalide. Tapez DELETE pour confirmer'}), 400
        
        # Supprimer le compte
        # Remplacer par la logique de suppression de compte si existante, sinon lever une erreur explicite
        return jsonify({'success': False, 'message': 'Suppression de compte non implémentée'}), 501
...
def reset_password():
    ...
    try:
        ...
        if success:
            ...
            return jsonify({'success': False, 'message': 'Erreur lors de la réinitialisation'}), 400
...
def get_login_history():
    ...
    try:
        ...
        logger.error(f"Erreur lors de la récupération de l'historique de connexion: {e}")
        return jsonify({'success': False, 'message': 'Erreur interne du serveur'}), 500 