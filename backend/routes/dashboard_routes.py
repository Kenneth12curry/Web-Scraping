...
def get_dashboard_stats():
    ...
    try:
        ...
        user_query = "SELECT id, subscription_plan, monthly_requests_limit FROM users WHERE username = %s"
        user_data = mysql_connector.execute_query(user_query, (current_user,))
        if not user_data:
            return jsonify({'success': False, 'message': 'Utilisateur non trouvé'}), 404
        user_id = user_data[0]['id']
        scraping_limit = user_data[0].get('monthly_requests_limit', 30)
        ...
        scraping = scraping_stats[0] if scraping_stats and len(scraping_stats) > 0 else {
            'total_scraping': 0,
            'total_articles': 0,
        ...
        recent_scraping_query = """
            ...
            LIMIT 10
        """
        recent_scraping = mysql_connector.execute_query(recent_scraping_query, (user_id,))
        return jsonify({
            'success': True,
            'user_stats': {
                ...
                'avg_articles_per_scraping': scraping.get('avg_articles_per_scraping', 0) or 0,
...
def get_analytics():
    ...
    try:
        ...
        recent_activity_query = """
            ...
            LIMIT 10
        """
        recent_activity = mysql_connector.execute_query(recent_activity_query, (user_id,))
        ...
        scraping_total = scraping_stats[0]['total_scraping'] if scraping_stats and len(scraping_stats) > 0 else 0
        scraping_success = scraping_stats[0]['successful_scraping'] if scraping_stats and len(scraping_stats) > 0 else 0
        ...
        scraping_trends_query = """
            ...
            ORDER BY date
        """
        scraping_trends = mysql_connector.execute_query(scraping_trends_query, (user_id,))
        ...
        for i in range(7):
            ...
            day_data = next((d for d in scraping_trends if d.get('date') and d['date'].strftime('%Y-%m-%d') == date), None)
            if day_data:
        ...
        return jsonify({
            ...
            'scraping_stats': scraping_stats[0] if scraping_stats and len(scraping_stats) > 0 else {
                'total_scraping': 0,
...
def get_weather():
    ...
    try:
        ...
        if api_key == 'demo_key':
            ...
            weather_conditions = [
                {'condition': 'Ensoleillé', 'icon': 'fas fa-sun', 'temp_range': (18, 28)},
                {'condition': 'Nuageux', 'icon': 'fas fa-cloud', 'temp_range': (15, 25)},
                {'condition': 'Pluvieux', 'icon': 'fas fa-cloud-rain', 'temp_range': (12, 22)},
            ...
            if response.status_code == 200:
                ...
                weather_data = {
                    ...
                    'icon': weather_icons.get(data['weather'][0]['main'], 'fas fa-cloud'),
                    'humidity': data['main']['humidity'],
                    'wind': round(data['wind']['speed'] * 3.6),  # m/s to km/h
                    'feels_like': round(data['main']['feels_like']),
                    'pressure': data['main']['pressure'],
                    'visibility': round(data.get('visibility', 10000) / 1000),  # m to km
                    'updated_at': datetime.now().isoformat()
                }
            else:
...
def get_notifications():
    ...
    try:
        ...
        user_query = "SELECT id, monthly_requests_used, monthly_requests_limit FROM users WHERE username = %s"
        user_data = mysql_connector.execute_query(user_query, (current_user,))
        if not user_data:
            return jsonify({'success': False, 'message': 'Utilisateur non trouvé'}), 404
        user_id = user_data[0]['id']
        used = user_data[0].get('monthly_requests_used', 0)
        limit = user_data[0].get('monthly_requests_limit', 30)
        ...
        scraping_activity_query = """
            ...
            LIMIT 3
        """
        scraping_activity = mysql_connector.execute_query(scraping_activity_query, (user_id,))
...
def get_exports():
    ...
    try:
        ...
        for export_item in exports_data:
            from urllib.parse import urlparse
            domain = urlparse(export_item['url']).netloc if export_item['url'] else 'unknown'
